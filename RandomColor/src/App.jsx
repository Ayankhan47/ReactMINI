...
const App = () => {
  ...
  const clickHandler = () => {
    ...
    buttonRef.current.innerHTML = value1 + ',' + value2 + ',' + value3;
    bulbRef.current.style.boxShadow = `5px 5px 20px rgba(${value1}, ${value2}, ${value3}, 0.5)`;
  ...
  return (
    <div className='bg-black h-screen flex items-center justify-center gap-36'>
      ...
      <button onClick={clickHandler} ref={buttonRef} className='py-3 px-6 rounded-xl active:scale-[0.97] bg-white shadow-sm shadow-white' role="button">Turn on</button>
    </div>
  );
}
